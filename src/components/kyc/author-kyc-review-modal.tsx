...
interface AuthorKycReviewModalProps {
  kyc: any;
}

function DocLink({ url, label }: { url: string | null; label: string }) {
  if (!url) return <span className="text-[10px] font-bold uppercase opacity-30 italic">Not provided</span>;
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[11px] font-black uppercase underline underline-offset-2 hover:text-accent transition-colors">
...
export function AuthorKycReviewModal({ kyc }: AuthorKycReviewModalProps) {
  ...
  const reviewMutation = trpc.reviewAuthorKyc.useMutation({
    onSuccess: (_, vars) => {
      toast({
        title: vars.decision === "approved" ? "Author KYC Approved" : "Author KYC Rejected",
        description: "Author has been notified by email.",
      });
      utils.getAllAuthorKycSubmissions.invalidate();
      setOpen(false);
    },
    onError: (err) =>
  ...
  const handleDecision = (decision: "approved" | "rejected") => {
    if (!reviewerId) {
      toast({
        ...
        description: "Your staff session is still loading. Please try again in a moment.",
      });
      return;
    }
    if (decision === "rejected" && !notes.trim()) {
      toast({ variant: "destructive", title: "Notes required", description: "Please provide rejection feedback for the author." });
      return;
    }
  ...
  const author = kyc.author;
  const user = author?.user;
  const publisherName = author?.publisher?.tenant?.name ?? "Unknown Publisher";
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto border-4 border-black rounded-none gumroad-shadow">
        <DialogHeader>
          <DialogTitle className="font-black uppercase italic tracking-tighter text-xl">
            Author KYC Review — {publisherName}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 pt-2">
          ...
          <div className="bg-white border-2 border-black p-5 space-y-4">
            <h4 className="font-black uppercase text-[10px] tracking-widest border-b border-black/10 pb-2 flex items-center gap-2">
              <Building2 className="size-3" /> Publishing Context
            </h4>
            <div className="grid grid-cols-2 gap-4">
              <InfoRow label="Publisher" value={publisherName} />
            ...
            <div className="pt-1">
              <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-2">Business Registration</p>
          ...
          <div className="space-y-2">
            <label className="font-black uppercase text-[10px] tracking-widest">
              Reviewer Notes
              {!isReviewed && <span className="ml-2 font-normal normal-case opacity-40">(required if rejecting)</span>}
            </label>
            <Textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              disabled={isReviewed || reviewMutation.isPending}
              placeholder="Explain what is missing or note the approval context…"
          ...
          {isReviewed ? (
            ...
            <div className="flex gap-3">
              <Button className="flex-1 h-14 border-4 border-black bg-white text-black font-black uppercase italic hover:bg-red-50 flex items-center justify-center gap-2" disabled={reviewMutation.isPending || sessionStatus !== "authenticated" || !reviewerId} onClick={() => handleDecision("rejected")}>
                <XCircle className="size-4" />
                Reject
              </Button>
              <Button className="flex-1 h-14 booka-button-primary flex items-center justify-center gap-2" disabled={reviewMutation.isPending || sessionStatus !== "authenticated" || !reviewerId} onClick={() => handleDecision("approved")}>
