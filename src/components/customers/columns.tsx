...
function CustomerAction({ customer }: { customer: any }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="rounded-none border-4 border-black gumroad-shadow w-56 p-0 bg-white"
      >
        ...
        <DeleteCustomerModal
          id={customer.id}
          trigger={
            <div className={cn(menuButtonStyle, "text-red-600")}>
...
// ── Name column — shown to everyone including authors ─────────
...
// ── Columns visible to non-authors only ───────────────────────
...
// ── Author-only view — name + a soft "restricted" note ────────
const authorColumns: ColumnDef<any>[] = [
  nameColumn,
  {
    ...
    header: () => null,
    cell: () => (
      <span className="text-[10px] font-bold uppercase tracking-widest opacity-20 italic">
...
// ── Column factory — used by the page ────────────────────────
export function buildCustomerColumns(isAuthor: boolean): ColumnDef<any>[] {
  return isAuthor ? authorColumns : fullColumns;
}

// Keep a default export for any existing imports that use it directly
// (non-author view as the safe default)
export const customerColumns = fullColumns;