...
export function RegisterForm() {
  ...
  const registerMutation = trpc.createUser.useMutation({
    onSuccess: () => {
      // ── KEY CHANGE: go to "check your inbox" page ──
      // Pass the email so the resend button can pre-fill
      router.push(
  ...
  const handleFinalSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userStatus?.available === false) {
      toast({
        ...
        description: "Choose another handle.",
      });
      return;
    }
    if (selectedRole === "Publisher" && slugStatus?.available === false) {
      toast({
        variant: "destructive",
        title: "Slug Taken",
        description: "This storefront URL is already reserved.",
      });
      return;
    }
      ...
      {step === 1 && (
        <div className="space-y-10">
          <div className="text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary/40 mb-4">
              Identity Selection
            </p>
            ...
            {roles.map((r) => (
              ...
              >
                ...
                <div className="mt-auto">
                  ...
                  <p className="text-[10px] font-bold uppercase mt-2 text-primary/60 leading-relaxed">
                    {r.desc}
                  </p>
                </div>
              </button>
      ...
      {step === 2 && (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
          <div className="text-center">
            <button
              onClick={() => setStep(1)}
              className="text-[10px] font-black uppercase tracking-widest text-primary/40 hover:text-primary underline mb-4"
            >
              ← Back
            </button>
            <h2 className="text-3xl font-black uppercase italic tracking-tighter">
              ...
              <div className="space-y-4 p-6 bg-accent border-4 border-black gumroad-shadow-sm mb-6">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    className="booka-input-minimal h-14 bg-white"
                    required
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black opacity-30 uppercase italic">
                      {baseUrl}/
                    </div>
                    <Input
                      value={slugInput}
                      ...
                      className={cn(
                        "booka-input-minimal h-14 pl-28 bg-white",
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      ...
                      ) : slugInput.length > 2 && slugStatus?.available ? (
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <Input
                  ...
                  className={cn(
                    "booka-input-minimal h-14",
                    userStatus?.available === false && "border-red-500 bg-red-50"
                  )}
                  required
                />
                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                  {isCheckingUser ? (
                    <Loader2 className="animate-spin size-4 opacity-40" />
                  ) : usernameInput.length > 2 && userStatus?.available ? (
                    <CheckCircle2 className="text-green-600 size-4" />
                  ) : usernameInput.length > 2 && !userStatus?.available ? (
                    <AlertCircle className="text-red-600 size-4" />
                  ) : null}
            ...
            <div className="pt-6">
              <Button
                ...
                disabled={
                  ...
                  (selectedRole === "Publisher" && !slugStatus?.available)
                }
